document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (id) {
    try {
      const url = `https://ander-villantoy.github.io/tarjeta/data/tarjetas.json`;
      ...
      function toggleEnvelope(open) {
        if (open) {
          if (tarjeta.tipo === 'formal') {
            ...
            new Audio("audio/open.wav").play(); // Ruta corregida
          }
          setTimeout(() => {
          ...
          if (tarjeta.tipo !== 'formal') {
            new Audio("audio/close.wav").play(); // Ruta corregida
          }
          setTimeout(() => {
